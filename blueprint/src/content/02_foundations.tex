...
\lean{MIPRE.xSqNorm, MIPRE.xPovmDist, MIPRE.IsXPOVMClose, MIPRE.POVM.mul_self_le_self,
  ...
  MIPRE.stateVecB_smul, MIPRE.stateVecB_sum}
\leanok
Let $\mu$ be a distribution on a finite set $\mX$, let
...
\lean{MIPRE.sgn, MIPRE.sgn_add, MIPRE.star_sgn, MIPRE.norm_sgn, MIPRE.sum_sgn,
  ...
  MIPRE.LowDegree.card_filter_ker_mul'}
\leanok
...
\end{enumerate}
Writing $\qp^Z_c$ for the projector onto the computational basis vector $c \in \F_q^n$, the
$\qp^Z(b)$ expand in them and are inverted by Fourier averaging:
\begin{equation}
  \label{eq:pauli-obs-proj-bp}
  \qp^Z(b) \;=\; \sum_{c} (-1)^{\langle b, c\rangle}\, \qp^Z_c\;, \qquad
  \E_{b \in \F_q^n} (-1)^{\langle b, c \rangle}\, \qp^Z(b) \;=\; \qp^Z_c\;.
\end{equation}
\end{definition}
...
What is not yet here: the Fourier transform conjugating the $\qp^Z$ family into the $\qp^X$ one
and hence the $X$-side projectors, the syndrome projectors
$\qp^W_{[\cdot\, v = a]}$ that Lemma~\ref{lem:qld-expanded-points} averages to, and
$\ket{\EPR_q}^{\ot M}$ with its stabilizer relations.
