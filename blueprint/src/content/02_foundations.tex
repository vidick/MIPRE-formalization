...
the synchronous framework the first two conditions come for free in the direction this
blueprint uses: the tensor-product realization of a synchronous strategy on the maximally
entangled state (the proof of Lemma~\ref{lem:sync-le-valstar}) is projective and
consistent (\cite[Remark 5.13]{JNVWY20}; see also \cite[Theorem 5.5]{PSSTW16}), so only
the
...
(Definition~\ref{def:cl-function}), and $\TIME_\sampler(n)$ denotes the maximal running time
of $\sampler$ over all inputs of the form $(n, \ldots)$.
\end{definition}

\paragraph{Comments.} This is the paper's \texttt{def:sampler} (\texttt{linear.tex}) at
field size $2$, the only size the normal form verifiers of this blueprint use; the paper's
samplers over $\F_{2^t}$ reach this form by downsizing (Section~\ref{sec:ps-cl}). Two points
the formalization fixes. The correctness clauses for $\mathtt{linear}$ and $\mathtt{factor}$
are imposed only for prefixes in the range of $L^w_{<j}$, as the paper's amended definition
states, and are well defined there by Lemma~\ref{lem:cl-structure}. And totality on all inputs
$(n, \ldots)$ is what makes $\TIME_\sampler(n)$ finite; the paper secures it by bounded format
checks that read only a prescribed prefix of each input. In Lean the machine is a program of
the ambient cost model (Section~\ref{sec:computability-conventions}), its input the pair of
$n$ and a query (\texttt{Sampler.Query}), and $\TIME_\sampler(n) \le T$ is read as halting
within cost $T \cdot (\abs{d} + 1)$ on every input $(n, d)$, well formed or not
(\texttt{Sampler.TimeBoundAt}). The paper's bound is uniform over all inputs, and that
reading is not available in the ambient model: a value is inspected in place one node at a
time, but a loop carries the rest of a list to its next iteration only by copying it, so no
program walks an unbounded list within a cost independent of its length, and a bound uniform
over all inputs is met by no sampler or decider that checks the length of a string against
an unbounded $s(n)$. The bound is therefore stated up to the cost of reading the input; on
the well-formed inputs of the game, whose size is bounded by the answer-length bound, it is a
polynomial bound in $T$, which is all the pipeline uses, and a machine that performs the
paper's format checks first meets it.
...
$\TIME_\decider(n)$ denotes its maximal running time over all inputs with index $n$. In Lean,
$\TIME_\decider(n) \le T$ is read as halting within cost $T \cdot (\abs{d} + 1)$ on every
input $(n, d)$ (\texttt{Decider.TimeBoundAt}), for the reason given after
...
(with field size $2$) and a decider such that, on input $(n, x, y, a, b)$, the decider first
computes the dimension $s(n)$ of the sampler and rejects unless $\abs{x} = \abs{y} = s(n)$.
For each $n$ with $\TIME_\decider(n) < \infty$, $\verifier$ determines the \emph{$n$-th game}
$\verifier_n$: its question sets are $\mX = \mY = \{0,1\}^{s(n)}$, its answer sets $\mA = \mB$
are the strings of length at most $\TIME_\decider(n)$, its question distribution is the
distribution of $\sampler$ at index $n$ (Definition~\ref{def:sampler}), and its decision
predicate is $D_n(x, y, a, b) = 1$ if and only if $\decider(n, x, y, a, b)$ accepts. We write
$\valstar(\verifier_n)$ for its quantum value. The game is \emph{synchronous} in the sense of
Definition~\ref{def:sync-game} when the decider rejects unequal answers to equal questions;
this is a condition on the decider (Remark~\ref{rem:sync-invariant}), and
$\synval(\verifier_n)$ then denotes the synchronous value. The description length is
$\abs{\verifier} = \max\{\abs{\sampler}, \abs{\decider}\}$, and the number of levels of
$\verifier$ is that of $\sampler$.
\end{definition}

\paragraph{Comments.} The paper's \texttt{def:normal-ver} and \texttt{def:normal-game}
(\texttt{games.tex}), including the amended question-length check, which the previous text
of this definition omitted; in the paper the check is what gives $s(n) \le \TIME_\decider(n)$
(\texttt{rmk:normal-verifier-question-length-check}). The formalization takes the
answer-length bound as a parameter: \texttt{Verifier.game n T} is $\verifier_n$ with answers
of length at most $T$, the paper's game being the case $T = \TIME_\decider(n)$; every statement
about $\verifier_n$ carries its time bound explicitly, as the paper's do
($\TIME_\decider(n) \le n^\lambda$, $\poly(2^{\lambda n}, \ell)$, and so on), and the exact
supremum would only add the degenerate case $\TIME_\decider(n) = \infty$, where the paper's
game is undefined, to every definition. Timeouts are not modelled: in the cost model of
Section~\ref{sec:computability-conventions} a decider with $\TIME_\decider(n) < \infty$ halts
on every input of index $n$, and acceptance is halting with output $1$. The question-length
check is kept in the form of its consequence for the game, that the decider accepts only
questions of length $s(n)$ (\texttt{Verifier.accepts\_length}); where the paper uses
$s(n) \le \TIME_\decider(n)$, the formalization requires $s(n) \le n^\lambda$ as a clause of
$\lambda$-boundedness (Definition~\ref{def:lambda-bounded}): with the running times bounded
up to the cost of reading the input (Definition~\ref{def:sampler}), neither
$s(n) \le \TIME_\decider(n)$ nor $s(n) \le \TIME_\sampler(n)$ is a consequence any more.
...
\item \emph{Diagonal weight}: the game to which the almost-synchronous transport is
  applied satisfies $\mu(x,x) \geq \kappa \sum_y \mu(x,y)$ for every $x$, with the same
...
all $n \geq 2$, and $\abs{\verifier} \leq \lambda$. In Lean (\texttt{Verifier.IsBounded}) the
two time bounds are read as in Definitions~\ref{def:sampler} and~\ref{def:decider}, halting
within cost $n^\lambda (\abs{d} + 1)$ on every input $(n, d)$, and the clause
$s(n) \le n^\lambda$ --- in the paper a consequence of $\TIME_\sampler(n) \le n^\lambda$,
the sampler having to write $s(n)$ output cells --- is required directly.
\end{definition}

\begin{definition}[The class $\MIPstar$]
\label{def:mipstar}
\ledgernode{1.1.7, 1.1.7.1}
\uses{def:normal-verifier,def:q-value,def:game-description}
\lean{MIPRE.MIPStar}
\leanok
...
The Lean definition (\texttt{MIPRE.MIPStar}) is the \emph{computable} version of the class,
with games given as explicit descriptions: a computable map $z \mapsto \game_z$ from strings
to game descriptions (Definition~\ref{def:game-description}) with the same completeness and
soundness clauses. Every language with a polynomial-time verifier whose games are
synchronous is in it --- the game at input $z$ is finite and computable from the verifier,
and for a synchronous game the description's rejection of unequal answers to equal
questions changes nothing --- and that is the shape Theorem~\ref{thm:halting} delivers
(Section~\ref{ch:main-theorem}); for a general verifier the tabulation preserves soundness,
but a value-$1$ witness that is inconsistent on equal questions need not survive it. The
computable version is what the Lean main statement asserts, the inclusion in $\RE$ is
formalized for it (Lemma~\ref{lem:mipstar-sub-re}), and the polynomial-time clause is not
formalized.
\end{definition}
