...
\mH_\bob$, and POVMs $\{A^x_a\}$ on $\mH_\alice$ and $\{B^y_b\}$ on $\mH_\bob$. 

Outcome probabilities are computed using the Born rule: the players answer $(a,b)$ to questions $(x,y)$
with probability $\bra{\psi} A^x_a \ot B^y_b \ket{\psi}$. 
\end{definition}
...
and $\tau = \tr/d$, with the same value.
\end{proof}
...
$\E_{x \sim \mu} \sum_a \tau((M^x_a - N^x_a)^2) \leq \delta$.
\end{definition}
