...
checks that read only a prescribed prefix of each input, a convention the Lean definition does
not have to spell out, since $\TIME_\sampler(n) \le T(n)$ is stated directly as halting within
cost $T(n)$ on every such input (\texttt{Sampler.TimeBound}). In Lean the machine is a program
of the ambient cost model (Section~\ref{sec:computability-conventions}), its input the pair of
$n$ and a query (\texttt{Sampler.Query}).
...
$\TIME_\decider(n)$ denotes its maximal running time over all inputs with index $n$.
\end{definition}
...
$s(n) \le \TIME_\decider(n)$, the formalization has $s(n) \le \TIME_\sampler(n)$ instead,
which follows from the sampler's marginal queries producing outputs of length $s(n)$
(\texttt{Sampler.dim\_le\_of\_timeBoundAt}).
