...
$n$ and a query (\texttt{Sampler.Query}), and $\TIME_\sampler(n) \le T$ \emph{at degree $k$}
is read as halting within cost $T \cdot (\abs{d} + 1)^k$ on every input $(n, d)$, well formed
or not (\texttt{Sampler.TimeBoundAt}).
...
$\TIME_\decider(n) \le T$ at degree $k$ is read as halting within cost
$T \cdot (\abs{d} + 1)^k$ on every input $(n, d)$ (\texttt{Decider.TimeBoundAt}), for the
...
by a polynomial in the size of the input (Definition~\ref{def:sampler}), neither
...
within cost $n^\lambda (\abs{d} + 1)^\lambda$ on every input $(n, d)$ --- so $\lambda$ bounds
both the coefficient and the degree of the polynomial dependence on the input size, which is
what makes the condition a polynomial-time one --- and the clause $s(n) \le n^\lambda$ --- in
the paper a consequence of $\TIME_\sampler(n) \le n^\lambda$, the sampler having to write
$s(n)$ output cells --- is required directly.
\end{definition}
