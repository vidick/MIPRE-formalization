...
\uses{def:observable,lem:observable-projector,def:lcs-instance}
...
\uses{def:lcs-instance,def:solution-group}
...
\uses{def:magic-square,def:observable}
...
\uses{def:lcs-game,def:tensor-strategy,def:q-value,lem:observable-strategy}
...
\begin{proof}
\leanok
Take both players' spaces to be $\C^n$ with the normalized EPR state. The game is
symmetric, so each player needs measurements at both kinds of question, and both use the
same ones: at an equation, the joint projective measurement of its observables, answered
as an assignment extended by zero; at a variable, the binary measurement of its
observable. At an equation--variable incidence the accepted Born-rule weight is the EPR
expectation of the local winning operator, which is $1$ because the local loss operator
annihilates the EPR vector. The other orientation has the same weight, since
$\langle \Omega, (A \otimes B) \Omega\rangle = \sum_{a,k} A_{ak} B_{ak}$ is symmetric in
$A$ and $B$ and the decider is symmetric in the players. No transposes are needed.
\end{proof}
...
\uses{def:game,def:state-distance,lem:qld-povm-to-obs,def:generalized-pauli}
...
step. \texttt{norm\_stateVec\_comm\_le} is~\eqref{eq:anticomm-transfer-bp} with a commutator in the middle
instead of an anticommutator --- the same split, Bob's two products cancelling either way. And
\texttt{snorm\_swapVec\_aOp\_sub\_bOp} exchanges the two sides of a cross-party deviation, which is what lets
...
\begin{proof}
\leanok
\uses{def:state-distance,lem:qld-combined-points,lem:commutation-analysis}
...
\uses{def:cross-distance,lem:commutation-analysis}
\lean{MIPRE.fibSum, MIPRE.isPVM_fibSum, MIPRE.sum_fiber, MIPRE.abs_sum_sum_le_sqrt,
  ...
  MIPRE.sum_weighted_add, MIPRE.sum_weighted_div, MIPRE.sum_weighted_const_mul,
  MIPRE.IsPVM.comp_equiv, MIPRE.qform_conjTranspose,
...
\begin{proof}
\leanok
\uses{lem:commutation-analysis,def:cross-distance,lem:qld-combined-points}
...
\uses{def:generalized-pauli,def:weyl-epr}
...
\begin{proof}
\leanok
\uses{def:generalized-pauli,def:weyl-epr,lem:self-dual-basis-exists,lem:downsize-field}
