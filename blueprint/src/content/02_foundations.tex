...
rejected. Symmetrizing the roles of the two players makes the question distribution and
the decision predicate symmetric; that $\verifier_n$ is moreover \emph{synchronous} in the
sense of Definition~\ref{def:sync-game} is a further condition on the decider, which must
reject unequal answers to equal questions, and not a consequence of symmetrization
(Remark~\ref{rem:sync-invariant}). We write $\valstar(\verifier_n)$ for the quantum value
of $\verifier_n$ and, when $\verifier_n$ is synchronous, $\synval(\verifier_n)$ for its
synchronous value. The
...
Two properties are asked of the games produced along the compression pipeline of
Section~\ref{ch:proof-structure}, and neither follows from symmetrizing the players'
roles:
\begin{enumerate}
\item \emph{Synchronicity}: equal questions reject unequal answers, so that $\synval$ is
  ...
  is a condition on the decider. It is needed wherever $\synval$ occurs --- in the
  soundness clauses of all four transformations and at the inner end of the recursion ---
  and direct repetition preserves it, since two unequal answer tuples to equal question
  tuples differ in some coordinate, where the repeated game rejects.
...
\end{enumerate}
The transport is applied once, to the game that the repetition step repeats --- the
answer-reduced game --- so the second property is a requirement on the constructions of
introspection, oracularization and answer reduction, and not something inherited from the
game being compressed. It is emphatically not a property of repeated games: the $k$-fold
repetition of a game with diagonal weight $\kappa$ has diagonal weight $\kappa^k$, which
is why the pipeline's soundness is carried in $\valstar$ from the repetition step outwards
and the transport is never applied to a repeated game. Establishing either property is
open: neither is recorded in the statements of Section~\ref{ch:proof-structure}, and
in~\cite{JNVWY20} both come from the typed structure of the samplers, which this blueprint
does not carry (Section~\ref{sec:departures}).
\end{remark}
