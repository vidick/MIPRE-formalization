...
\emph{Status.} The theorem is proved in Lean conditionally on Theorem~\ref{thm:compression}:
...
difference between the two is one structure.
...
(\texttt{MIPRE.Halting.exists\_obligations}), so nothing but Theorem~\ref{thm:compression}
...
\begin{proof}
\leanok
Conditionally on Theorem~\ref{thm:compression}, as Theorem~\ref{thm:main} is:
...
(\texttt{Halting/Corollaries.lean}), conditionally on Theorem~\ref{thm:compression}.
\end{proof}
...
Definition~\ref{def:mipstar}; the equality is conditional on Theorem~\ref{thm:compression},
