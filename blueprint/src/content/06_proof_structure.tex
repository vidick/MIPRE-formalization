...
repetition (Theorem~\ref{thm:direct-repetition-q}, formalized) restores soundness
...
\uses{def:normal-verifier,def:pcc,def:direct-repetition,thm:direct-repetition-q,thm:almost-sync,lem:sync-le-valstar}
There are universal constants $b, c, c' > 0$ and a polynomial-time Turing machine
$\ComputeParrepVerifier$ that on input $(\verifier, \lambda, \tau)$ outputs a normal
form verifier $\vrep$ whose game $\vrep_n$ is the $k(n)$-fold direct repetition
(Definition~\ref{def:direct-repetition}) of $\verifier_n$, with $k(n) = (\lambda n)^{(1 +
c')\tau}$, such that if $\verifier$ is $\ell$-level then $\vrep$ is $\ell$-level with
$\TIME_{\srep}(n) = O(k(n) \TIME_\sampler(n))$ and $\TIME_{\drep}(n) = O\bigl(k(n)
\max(\TIME_\decider(n), (\lambda n)^\tau)\bigr)$, the sampler of $\vrep$ depends only on
...
\item (\textbf{Soundness}) For all $\eps > 0$, if $\synval(\verifier_n) \leq 1 - \eps$
  ...
  $\synval(\vrep_n) \leq \exp\bigl( -c\, \eps^{b}\, k(n) / (\lambda n)^{\tau} \bigr)$;
  in particular $\synval(\vrep_n) \leq \frac12$ whenever $\eps \geq (\lambda
...
a value-$1$ PCC strategy is a value-$1$ PCC strategy for the repeated game. Soundness: by
the almost-synchronous transport (Theorem~\ref{thm:almost-sync}), $\synval(\verifier_n)
\leq 1 - \eps$ gives $\valstar(\verifier_n) \leq 1 - \eps'$ with $\eps' =
\Omega(\eps^{a})$ for a universal $a$; Theorem~\ref{thm:direct-repetition-q} bounds
$\valstar(\verifier_n^{k(n)})$, where the answer alphabets of $\verifier_n$ have size at
most $2^{\TIME_\decider(n)}$ so that $\log(\abs{\mA}\abs{\mB}) \leq 2 (\lambda n)^\tau$;
and $\synval \leq \valstar$ (Lemma~\ref{lem:sync-le-valstar}) concludes, with $b = 13 a$.
...
form verifier games with a value-$1$ PCC strategy and $B$ the set of those with $\synval
\leq \frac12$, with the trivially accepting and the trivially rejecting game as the
distinguished elements; the complement of $B$ is semidecided by enumerating synchronous
strategies (Lemma~\ref{lem:value-lower-approx}); and a description $(\sampler, \decider,
n)$ with $n$ in binary is precisely a succinct description of the game $\verifier_{2^n}$
played by a $\lambda$-bounded verifier. Completeness of compression is the preservation
of $A$ and soundness the preservation of $B$. Making this dictionary precise (in
particular the encoding of games by verifier descriptions and the role of $\lambda$) is a
formalization task of this section. The entanglement form of~\cite{JNVWY20}
...
oracularization; $\Ent(\vans_n, 1 - \eps) \geq \Ent(\verifier_n, 1 - \delta(\eps, n))$
for answer reduction; Theorem~\ref{thm:bvy} for anchored repetition; and, composed,
...
\label{thm:halting}
\uses{thm:compression,rem:compression-abstract,lem:compressible-criterion,lem:value-lower-approx,lem:kleene,def:mipstar,def:game-description}
