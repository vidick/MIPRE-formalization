...
repetitions, at polynomial cost in complexity and $+2$ levels. In value form the same is
achieved by direct repetition, without anchoring (Theorems~\ref{thm:direct-repetition-q}
and~\ref{thm:direct-repetition-co}, both formalized);
Remark~\ref{rem:direct-vs-anchored} discusses the two forms.
...
\item (\textbf{Completeness}) If $\verifier_N$ has a value-$1$ PCC strategy, then so
  does $\vcompr_n$.
\item (\textbf{Soundness}) $\Ent\bigl(\vcompr_n, \tfrac12\bigr) \geq \tfrac12
  \max\bigl\{ \Ent\bigl(\verifier_N, \tfrac12\bigr),\ 2^{N^{\lambda} - 1} \bigr\}$.
\end{enumerate}
\end{theorem}
...
a $\lambda$-bounded verifier. The entanglement bound above then gives $f(\compr(\hat{x},
n)) \geq \frac12 \max\{f(x), 2^{N^\lambda - 1}\}$ with $N = 2^n$, which is the
hypothesis of Lemma~\ref{lem:recursive-compression} up to the factor $\frac12$
of~\cite[Theorem 12.1]{JNVWY20}. The factor is harmless because the second term grows
much faster than the levels of the recursion: after $j$ levels the iteration in the
proof of the lemma gives $f(g(e)) \geq 2^{-j-1}\, 2^{N_j^\lambda - 1}$ with $N_j =
2^{n_j}$ and $n_j \geq 2^j r(e)$, which still tends to infinity with $j$. The lemma is
stated in the lossless form, so this instantiation requires the variant with hypothesis
$f(y) \geq \frac12 \max\{f(x), G(n)\}$ for a function $G$ growing faster than the
levels; that variant, with the same proof, is a pending strengthening of the Lean
statement. Making the rest of the dictionary precise (in particular the encoding of
games by verifier descriptions and the role of $\lambda$) is likewise a formalization
