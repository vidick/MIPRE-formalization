...
Theorem~\ref{thm:compression}, whose soundness bound carries a factor $\frac12$, so the
application needs the variant of the lemma tolerating such a loss per level
(Remark~\ref{rem:compression-abstract}). Note that~\cite{MNY} also proves a converse (their
