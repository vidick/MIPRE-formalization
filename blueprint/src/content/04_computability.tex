...
\uses{lem:bounded-universal-machine}
...
additive and linear in $bs{x}$ and the input size, in both directions, and the
description of $s(e, x)$ is that of $e$ plus $bs{x}$ plus a constant. The map $s$
...
Let $eta$ be the fixed \emph{bit-query} program that, on input $(((c', e), n), m)$,
runs the universal machine on $c'$ with input $(e, n)$ to obtain a string $y$ and outputs
the $m$-th bit of $y$ (or $2$ if $m \geq bs{y}$). Let $a$ be the polynomial-time
\emph{decider} that on $(c', (e, n))$ outputs a fixed $y_0 \in A$ if $e$ halts on the
empty input within $\log n$ steps, and $\comprigl((s(eta, ((c', e), 2n+1)), n),
nigr)$ otherwise, where $s$ is the s-m-n map of Lemma~ef{lem:smn}. By
Lemma~ef{lem:kleene} there is a program $c$ with $\phi_c(e, n) = \phi_a(c, (e, n))
=: h(e, n)$, running in time polynomial in $bs{e} + \log n$. Hence for $n$ above a
threshold $r(e) = 2^{K + 1 + bs{e}}$ (with $K$ a constant depending only on the
polynomial overheads), $(s(eta, ((c, e), 2n+1)), n)$ is a succinct description of
$h(e, 2n+1)$: the string has length at most $2^n$ and the bit-query program answers within
the budget of Definition~ef{def:succinct}. The reduction is $g(e) = h(e, r(e))$. If $e$
