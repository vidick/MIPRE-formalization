...
\uses{thm:halting,def:co-value-bipartite,def:q-value,lem:valco-upper-re,thm:compression}
...
The argument is a dichotomy against the semidecider of
Lemma~\ref{lem:valco-upper-re}. If the commuting value is $1$ the semidecider never halts, the
early-accept branch is absent, and compression drives $\Ent(\verifier^{\sep}_n, \frac12)$ above
every bound, giving $\valstar \leq \frac12$; if it is less than $1$ the semidecider halts, the
early-accept branch supplies a value-$1$ PCC strategy at every level, and $\valstar = 1$
contradicts $\valstar \leq \omegaco < 1$. So the value proved equal to $1$ is exactly the value
the semidecider is complete for --- and that semidecider is the NPA hierarchy, whose limit is
the bipartite set $C_{qc}$. Nothing in the argument exhibits a strategy: as the source puts it,
the proof does not produce an explicit value-$1$ commuting-operator strategy.

Since $\valco \leq \omegaco$ with the reverse comparison false in general
(Lemma~\ref{lem:tracial-le-co}), $\valco(\game^{\sep}) = 1$ would be a \emph{stronger}
statement, and this route does not prove it: it would need a hierarchy converging to the tracial
value from above, which is a separate question and not what the source uses. Earlier drafts of
this chapter stated the theorem with $\valco$, which overclaimed. Everything downstream is
unaffected --- Corollary~\ref{cor:tsirelson} separates the correlation sets $C_{qa}$ and
$C_{qc}$, which is a statement about $\omegaco$'s model --- so the repair is to the statement,
...
\omegaco$ for all games, then combining Lemma~\ref{lem:value-lower-approx} (value
