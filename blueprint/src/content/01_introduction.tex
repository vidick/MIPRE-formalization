...
\emph{Status (September 2026).} The main theorem, and with it $\MIPstar = \RE$ and the
uncomputability of the game value, are proved in Lean \emph{conditionally on gap-preserving
compression}, Theorem~\ref{thm:compression}, stated as the structure
\texttt{MIPRE.GapCompression}: \texttt{MIPRE.Halting.halting\_reduces\_to\_gameValue\_of}
takes that one structure and concludes Theorem~\ref{thm:main} with no \texttt{sorry}, and
\texttt{\#print axioms} shows nothing else. The unconditional statement keeps its
\texttt{sorry} as the target. The abstract compression assembly, the repetition
instance and the ambient introspection instance for seven-level inputs are
formalized. Supplying answer reduction (including its internal typed
oracularization) remains open, with the paper-to-model interfaces recorded in
Section~\ref{ch:proof-structure}. The goals of this project are to (a) complete a formalization of this theorem and
...
property and the property of having value at most $\frac12$. Given compression, an
abstract fixed-point argument (Lin's \emph{compressibility criterion},
...
amplification} by parallel repetition (anchored in~\cite{JNVWY20}, direct here). Each of these steps has one central
ingredient that can be formalized in an independent manner: the low individual degree
test, the PCP theorem (in a specific form), and a parallel repetition theorem
(Section~\ref{ch:parallel-repetition}; the direct, value-form theorems are formalized).
...
\paragraph{Bipartite soundness and synchronous completeness.}
The compression pipeline keeps the paper's bipartite tensor-product value $\valstar$
throughout (Remark~\ref{rem:bipartite-route}). Synchronous strategies and the value
$\synval$ provide the vocabulary for the final Lean statement and the doubled-game
completeness witnesses; they do not replace arbitrary bipartite strategies in the inner
soundness proofs. Consequently the pipeline does not invoke almost-synchronous transport
to recover a gap from $\synval$. That transport remains part of the paper's tensor-code
route, with its explicit marginal hypotheses, and is separate from the alternative
formalized classical low-degree theorem. A bridge between the paper's and the
formalization's PCC conventions must preserve the stated completeness properties, not
merely game value (Definition~\ref{def:pcc}).

\paragraph{Projective strategies via orthonormalization.}
The orthonormalization lemma (Section~\ref{sec:rr-orthonormalization}) replaces
almost-projective POVMs by nearby projective measurements on the \emph{same} space when
its quantitative hypothesis has been established. This is distinct from Naimark dilation,
which enlarges the space and is available where a bipartite argument permits it. Each
soundness lemma must state the strategy class it accepts and pay for any rounding step;
projectivity is not a blanket assumption on arbitrary near-optimal strategies.
...
\paragraph{Abstract compression, in value form.}
The fixed-point argument that turns compression into a halting reduction is stated once,
abstractly, as Lin's compressibility criterion~\cite{Lin25}
(Section~\ref{sec:rr-compression}): a compression procedure preserving ``perfect PCC
strategy'' and ``quantum value at most $\frac12$'', together with the semidecidability of
``quantum value above $\frac12$'', yields the reduction. This isolates all uses of Kleene's recursion
theorem in one quantum-free lemma, and it removes the entanglement bookkeeping
of~\cite{JNVWY20} from the pipeline: no transformation needs an entanglement clause, and
gap amplification uses direct rather than anchored parallel repetition, for which complete
formalizations exist (Section~\ref{sec:pr-direct}). The recursive compression lemma of
Marks--Nezhadi--Yuen~\cite{MNY}, instantiated with $f = $ entanglement requirement, is
retained as the entanglement-form alternative (Remark~\ref{rem:entanglement-form}).

\paragraph{Other candidate simplifications (to be decided).}
Using de la Salle's quantitative improvements to stability (Gowers--Hatami) where they
shorten proofs; other simplifications that are present in the follow-up proof of
...
(\verb|\lean| tags) identify the actual declarations; a statement-level \verb|\leanok|
records a formalized statement, and a proof-level mark records a guarded proof.
An implication from a transformation structure remains conditional until that structure
is supplied. Sections~\ref{ch:required-results}--\ref{ch:parallel-repetition} distinguish
formalized results, source theorems and open adapters individually; their common placement
does not make them all imports or all proved. Numerical constants in headline statements (error exponents)
are copied from~\cite{JNVWY20} and should be treated as provisional until the corresponding
section is worked out in detail. The level budget is an exception: it is derived here, and
the $7$ levels of Theorem~\ref{thm:compression} differ from the paper's $9$ precisely
because direct repetition adds no anchoring level.
