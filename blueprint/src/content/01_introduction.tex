...
\texttt{sorry} as the target. What remains is the proof of compression itself
(Section~\ref{ch:proof-structure}): introspection, oracularization, answer reduction and
parallel repetition, assembled by the margin arithmetic of Section~\ref{sec:ps-compression}. The goal of this projects are to (a) complete a formalization of this theorem and
...
for synchronous games whose questions carry proportional weight on the diagonal, the two
values agree closely enough to transport soundness inwards. That hypothesis is the
synchronization invariant of Remark~\ref{rem:sync-invariant}, which symmetrizing the two
players' roles does not by itself deliver, and it is why the soundness of the pipeline is
stated in $\valstar$ from the repetition step outwards
(Remark~\ref{rem:almost-sync-hypothesis}). The departure is also linked to the next one:
with synchronous strategies symmetry is definitional, which removes the factor $\frac12$
that the paper's symmetrization step costs (Remark~\ref{rem:entanglement-form}), while the
tracial framework is what makes Naimark dilation unattractive.
...
Sections~\ref{ch:required-results}--\ref{ch:parallel-repetition} are external results: the project may initially
...
obligations of the project. Numerical constants in headline statements (error exponents)
