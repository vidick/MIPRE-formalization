...
with Lin's tracial density theorem. These theorems are formalized: two external Lean
developments proving them are vendored into this repository, and the section records
their statements in the vocabulary of Section~\ref{ch:foundations} together with the
bridges connecting the two. Sections~\ref{sec:ar-background}--\ref{sec:ar-decomposition}
...
That theorem is a blueprint-only external result, which the pipeline of
Section~\ref{ch:proof-structure} invokes as long as its soundness clauses are stated
through the entanglement requirement (Definition~\ref{def:ent});
Remark~\ref{rem:direct-vs-anchored} explains how the value-form theorems of
Section~\ref{sec:pr-direct} can replace it. Section~\ref{sec:ar-background} collects
...
(from $\omegaco$ to $\valco$, which is false in general and true up to error for
synchronous games) is the commuting-operator analogue of
...
dimension. The pipeline of Section~\ref{ch:proof-structure} follows~\cite{JNVWY20} in
stating soundness through $\Ent$, and the gap amplification step
(Theorem~\ref{thm:parallel-repetition}) therefore invokes Theorem~\ref{thm:bvy}. Lin's
proof of $\MIPco = \coRE$~\cite{Lin25} shows that the entanglement clauses are
dispensable: with completeness and soundness of compression stated in value form
(perfect strategies are preserved, value at most $\frac12$ is preserved), the halting
reduction follows from the enumeration of Lemma~\ref{lem:value-lower-approx} and
Kleene's recursion theorem alone, and gap amplification then needs only a value
statement --- exactly the theorems of this section, applied to the game as it is,
without anchoring. Restating the pipeline in that form would retire
Theorem~\ref{thm:bvy} and its toolkit
(Sections~\ref{sec:ar-background}--\ref{sec:ar-decomposition}) from the proof of the
main theorem; it is listed among the candidate simplifications of
Section~\ref{sec:departures}. In either form, the transport between bipartite and
synchronous strategies (Section~\ref{sec:departures}) is the pipeline's responsibility,
as it is for Theorem~\ref{thm:bvy}.
\end{remark}
...
compression recursion requires when it is stated, as in~\cite{JNVWY20}, in terms of
$\Ent$ (Remark~\ref{rem:direct-vs-anchored}).
...
repetition is formalized (Section~\ref{sec:pr-direct}), and
Remark~\ref{rem:direct-vs-anchored} discusses when it can replace this theorem. Two
