...
both are formalized end to end, the entangled one through
...
\ledgernode{1.4.1.1, 1.4.1.2, 1.4.1.3}
...
The companion campaign no longer treats this theorem as an import: node $1.4.2$ was
decomposed in September 2026 into some sixty sub-nodes, down to the quantum
relative-entropy data-processing inequality the argument rests on, which is derived there
rather than cited --- from the integral representation of $-\ln$, joint convexity, and a
Weyl twirl realising the partial trace as a convex combination of unitary conjugations. That
is a legitimate route for the finite-dimensional partial-trace case and it is the one a
formalization would want, since it needs no operator-interpolation machinery. It does not
change this theorem's status \emph{here}, where it is unused.
