...
\uses{def:game-description,def:sync-value,def:q-value,def:re,lem:rational-strategies-suffice}
...
$\valstar$ half is Lemma~\ref{lem:rational-strategies-suffice} plus an enumeration: the
valid exact strategies with entries in $\mathbb{Q}(i)$ can be enumerated, validity is a
finite conjunction of polynomial identities in the entries, and the value is a rational
function of them, so ``some enumerated strategy has value $> p$'' is semi-decidable in
exact arithmetic. The $\synval$ half needs its own enumeration and density argument, since
rounding the entries of a synchronous strategy preserves neither projectivity nor
synchronicity; the pipeline does not consume it (Remark~\ref{rem:compression-abstract}),
...
proved but no longer on the path; Lemma~\ref{lem:perturbation-split} is used as stated. The
