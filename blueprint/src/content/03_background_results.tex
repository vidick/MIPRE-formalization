...
\noindent Theorem~\ref{thm:bvy} and the results of Section~\ref{sec:ar-background} are no
longer on the path to the main theorem (Remark~\ref{rem:direct-vs-anchored}).
...
Two claims about the source, from the verification campaign~\cite{Audit26} and
\emph{not} checked against~\cite{dlS21} here (that text is not vendored in this
repository). First, the statement above is over-hypothesized: the source is reported to
need only a \emph{normal} state, not a normal faithful tracial one, to give constant $9$
and exponent $1$, and to return a full projective measurement in the algebra. The tracial
hypothesis is not harmless bookkeeping --- it excludes the bipartite vector-state
applications that several of the projectivization sites of Section~\ref{ch:proof-structure}
need, so weakening it would turn three of those sites into genuine improvements while the
rest would still need Naimark dilation or the projective-by-definition convention of
Section~\ref{sec:departures}. Second, the attribution of the weaker predecessor
to~\cite{KPS18} and~\cite{JNVWY20} is reported to be wrong, the source crediting the low
individual degree paper and Kempe--Vidick instead. Both should be settled against the
source before this lemma is relied on in that generality; the campaign also offers a
consistency-to-projectivity step giving $O(\delta)$ in place of $O(\delta^{1/4})$, which
would sharpen the proof of Theorem~\ref{thm:almost-sync}.
...
\uses{thm:ms-rigidity,lem:ms-direct-anticomm,lem:self-dual-basis,thm:bvy,thm:direct-repetition-q,rem:direct-vs-anchored}
...
\begin{enumerate}
\item \textbf{Anchored parallel repetition} (node $1.4.2$, Theorem~\ref{thm:bvy}) is no
  longer admitted: the campaign decomposed it into a $60$-node subtree, and the quantum
  relative-entropy data-processing inequality it rested on is derived there rather than
  imported --- by joint convexity plus a Weyl twirl realising the partial trace as a convex
  combination of unitary conjugations, which is legitimate for the finite-dimensional
  partial-trace case. It remains \emph{unused} here regardless, since direct repetition
  (Theorem~\ref{thm:direct-repetition-q}) replaces it and takes a value hypothesis rather
  than an entanglement one (Remark~\ref{rem:direct-vs-anchored}).
