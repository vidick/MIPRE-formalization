...
\uses{thm:ms-rigidity,def:q-value,def:magic-square}
...
paper's own, and the tensor code theorem it reduces to is no longer an import either: the
companion campaign has decomposed that theorem's own proof (see
Remark~\ref{rem:lidt-formalized}). This matters for reading the Lean
...
\begin{proof}
From~\cite{JNVWY21tensor}, in the published form. The source is vendored and was audited in
full, and its internal derivation is now decomposed in the ledger rather than taken on trust
--- which is why the statements below carry their own sub-nodes. The statement above is the
synchronous form instantiated at the free parameter, and
Lemma~\ref{lem:tensor-codes-bipartite} is the two-prover form the reduction actually consumes.
\end{proof}
...
convex and non-decreasing, and suppose that every symmetric projective-measurement strategy
$T = (\ket{\Phi}, Q)$ for $\game$, of value $\omega(T)$, admits complete auxiliary POVMs
$N^y$ with
\begin{equation*}
...
let $g(a) = \{a\}$, and let $\kappa = 1$. Every symmetric projective-measurement strategy
meets the auxiliary hypothesis, using the transpose of its question-$1$ measurement. But the
...
\uses{thm:tensor-codes,cor:sync-transfer,rem:sync-transfer-marginal}
The two-prover form of Theorem~\ref{thm:tensor-codes} follows from the synchronous form by
symmetrizing the strategy and then applying Corollary~\ref{cor:sync-transfer} with a
concavified error function, at $R = \tfrac94$ as
...
supplies it. Since the corollary returns a POVM, the codeword measurement is projectivized
afterwards, in the ambient algebra, by Theorem~\ref{thm:orthonormalization}.
\end{lemma}
...
codewords is proved in the paper too, importing nothing. So the assumption was narrower than the
statement: one node, $1.2.1.7$, carried the tensor-code theorem itself. As of September 2026
not even that is an import --- the node has been restated as audited soundness with its
internal derivation decomposed in the ledger, which is what
Lemmas~\ref{lem:tensor-codes-induction} to~\ref{lem:tensor-codes-bipartite} and
Corollary~\ref{cor:sync-transfer} account for. The Lean statement is not an axiom, and
...
\uses{thm:qld,thm:ms-rigidity,lem:ms-direct-anticomm,thm:lidt-soundness,def:distance}
...
\uses{thm:ms-rigidity,lem:ms-direct-anticomm,lem:self-dual-basis,thm:direct-repetition-q,rem:direct-vs-anchored}
...
So the honest summary of what this blueprint assumes beyond Mathlib is now one classical
algorithmic result: a deterministic polynomial-time construction of irreducible polynomials
over $\F_2$. Nothing quantum, and no rigidity theorem. The mechanical figure is in
