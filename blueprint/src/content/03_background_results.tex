...
\ledgernode{1.2.1, 1.2.1.1, 1.2.1.2, 1.2.1.3, 1.2.1.4, 1.2.1.5, 1.2.1.6, 1.2.1.7, 1.2.1.7.1,
1.2.1.7.1.1, 1.2.1.7.1.2, 1.2.1.7.1.3, 1.2.1.7.2, 1.2.1.7.2.1, 1.2.1.7.2.2,
1.2.1.7.2.3, 1.2.1.7.2.4, 1.2.1.7.3, 1.2.1.7.4, 1.2.1.7.5, 1.2.1.7.6, 1.2.1.7.7,
1.2.1.7.8, 1.2.1.8, 1.2.1.9, 1.2.1.9.1, 1.2.1.9.2, 1.2.1.9.3}
...
The ledger marks node $1.2.1$ \emph{imported}: in the paper the quantum soundness of the
simultaneous low-degree test is obtained by reducing to the two-prover tensor-code test
of~\cite{JNVWY21tensor}, which is not reproved. The Lean statement is not an axiom, and
...
One node of the subtree is a warning rather than a step. Node $1.2.1.7.2.4$ records that
the interface between the product-basis transposes of~\cite{JNVWY20} and the
Schmidt-diagonal symmetric strategies of~\cite{Vid22} is \emph{false} as stated, with an
explicit $2 \times 2$ counterexample, and needs the approximate Takagi bridge instead. It
does not affect Theorem~\ref{thm:lidt-soundness}, whose Lean proof does not go through
that interface, but it is a live hazard for anything that reuses the transpose trick.
\end{remark}

\begin{theorem}[Pauli basis test / quantum low individual degree test]
\label{thm:qld}
\ledgernode{1.2.2, 1.2.2.1, 1.2.2.2, 1.2.2.3, 1.2.2.3.1, 1.2.2.4, 1.2.2.4.2, 1.2.2.5, 1.2.2.6,
1.2.2.7, 1.2.2.7.1, 1.2.2.7.2, 1.2.2.7.3, 1.2.2.8, 1.2.2.9, 1.2.2.10, 1.2.2.11,
1.2.2.12, 1.2.2.13, 1.2.2.14, 1.2.2.15, 1.2.2.16, 1.2.2.17}
...
sits. One of them was the campaign's only \emph{critical} finding, at node $1.2.2.15$: the
construction of exact Pauli operators rested on an identity between a codeword's
coordinates and the polynomial's values that is simply false, and had to be replaced by a
Schwartz--Zippel bound on the non-multilinear outcome mass. A formalization should treat
the appendix as the real content and the main-text statement as its interface.

Third, the introspection statement that consumes all of this (node $1.2$,
Theorem~\ref{thm:introspection}) is the one place in the pipeline where the soundness
guarantee is only $1 - \eps$ for a universal constant $\eps$ rather than a bound that can
be driven down, which is exactly why parallel repetition has to follow it.
\end{remark}
