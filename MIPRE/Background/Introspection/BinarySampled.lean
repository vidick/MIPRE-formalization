...
def sampleOp (hm : m ∣ Fintype.card F) (hL : ∀ w, (L w).SupportedOn univ)
    (c : QLD.Content F m) : QuestionType QLD.Ty ℓ → Answer F A m t d →
      Matrix (Space L D R) (Space L D R) ℂ
  | .inl p => pauliOp β L D R hm (c.question hm p)
...
theorem pauli_pair_commute (hm : m ∣ Fintype.card F) (c : QLD.Content F m)
    ...
    Commute (pauliOp β L D R hm (c.question hm p) a)
      (pauliOp β L D R hm (c.question hm q) b) := by
  cases a <;> cases b <;>
    simp only [pauliOp, Honest.parsedPauliOp, Commute.zero_left, Commute.zero_right]
  change _ * _ = _ * _
  rw [← pauliLift_mul, ← pauliLift_mul]
  exact congrArg (pauliLift β L D R) (QLD.Honest.answerOp_sampled_commute hm c p q h _ _).eq

theorem pauli_pair_reject (hm : m ∣ Fintype.card F) (hd : 1 ≤ d) (c : QLD.Content F m)
    (p q : QLD.Ty) (h : QLD.adj p q = true) (a b : Answer F A m t d)
    (hr : sampleCheck β L D hm c (.inl p) (.inl q) a b = false) :
    pauliOp β L D R hm (c.question hm p) a * pauliOp β L D R hm (c.question hm q) b = 0 := by
  cases a <;> cases b <;> simp only [pauliOp, Honest.parsedPauliOp, zero_mul, mul_zero]
  rename_i a b
  rw [← pauliLift_mul]
  apply (congrArg (pauliLift β L D R) ?_).trans (pauliLift_zero β L D R)
  by_cases ht : p = q
  · subst q
    by_cases hab : a = b
    · subst b
...
theorem sampleOp_commute (hβ : LowDegree.IsSelfDualBasis β) (hm : m ∣ Fintype.card F) (hL : ∀ w, (L w).SupportedOn univ)
    (hR : R.IsPCC) (c : QLD.Content F m) (p q : QuestionType QLD.Ty ℓ)
    (h : TypeGraph.Adj QLD.adj (.pauli .X) (.pauli .Z) p q) (a b : Answer F A m t d) :
    Commute (sampleOp β L D R hm hL c p a) (sampleOp β L D R hm hL c q b) := by
  cases p with
  | inl p =>
    cases q with
    ...
    | inr q =>
      ...
      · exact anchor_X_commute β L D R hβ hm hL w k hk a b
      · exact anchor_Z_commute β L D R hβ hm hL w a b
  | inr p =>
    cases q with
    | inl q =>
      ...
      rcases (TypeGraph.adj_pauli_aux_iff QLD.adj (.pauli .X) (.pauli .Z) q p w).mp hs with
        ⟨rfl,k,rfl,hk⟩ | ⟨rfl,rfl⟩
      · exact (anchor_X_commute β L D R hβ hm hL w k hk b a).symm
      · exact (anchor_Z_commute β L D R hβ hm hL w b a).symm
    | inr q =>
      change _ * _ = _ * _
...
theorem sampleOp_reject (hβ : LowDegree.IsSelfDualBasis β) (hm : m ∣ Fintype.card F) (hd : 1 ≤ d)
    (hL : ∀ w, (L w).SupportedOn univ) (hR : R.IsPCC) (hval : R.value = 1)
    (c : QLD.Content F m) (p q : QuestionType QLD.Ty ℓ)
    (h : TypeGraph.Adj QLD.adj (.pauli .X) (.pauli .Z) p q) (a b : Answer F A m t d)
    (hr : sampleCheck β L D hm c p q a b = false) :
    sampleOp β L D R hm hL c p a * sampleOp β L D R hm hL c q b = 0 := by
  cases p with
  | inl p =>
    cases q with
    ...
    | inr q =>
      ...
      rcases (TypeGraph.adj_pauli_aux_iff QLD.adj (.pauli .X) (.pauli .Z) p q w).mp h with
        ⟨rfl,k,rfl,hk⟩ | ⟨rfl,rfl⟩
      · exact anchor_X_reject β L D R hβ hm hL w k hk _ a b hr
      · exact anchor_Z_reject β L D R hβ hm hL w _ a b hr
  | inr p =>
    cases q with
    | inl q =>
      rw [(sampleOp_commute β L D R hβ hm hL hR c _ _ h a b).eq]
      have hr' : sampleCheck β L D hm c (.inl q) (.inr p) b a = false := by
      ...
      rcases (TypeGraph.adj_pauli_aux_iff QLD.adj (.pauli .X) (.pauli .Z) q p w).mp hs with
        ⟨rfl,k,rfl,hk⟩ | ⟨rfl,rfl⟩
      · exact anchor_X_reject β L D R hβ hm hL w k hk _ b a hr'
      · exact anchor_Z_reject β L D R hβ hm hL w _ b a hr'
    | inr q =>
      simp only [sampleOp, auxOp, ← aOp_mul]
      exact (congrArg (aOp (HB := Fin 2))
        (Honest.auxOp_reject_zero L D R hL hR hval QLD.adj (.pauli .X) (.pauli .Z)
          (project β) _ p q h a b hr)).trans aOp_zero
