...
qubit. Original CL seeds are arbitrary binary CL functions on the full qubit register; no executable
...
def game (hm : m ∣ Fintype.card F) (b : Module.Basis (Fin t) (ZMod 2) F) :=
  TypedEstimates.parsedGame QLD.adj (.pauli .X) (.pauli .Z)
    (QLD.PauliCL.binaryPresentation hm b) L (project (d := d) b) D (pauliCheck hm b)
...
def op (hm : m ∣ Fintype.card F) (b : Module.Basis (Fin t) (ZMod 2) F)
  ...
  match q.1 with
  | .inl p => pauliOp b L D R hm (QLD.PauliCL.binaryQuestion b p q.2)
...
theorem op_isPVM (hm : m ∣ Fintype.card F) (b : Module.Basis (Fin t) (ZMod 2) F)
  ...
  rcases q with ⟨q,x⟩
  cases q with
  | inl p => exact pauliOp_isPVM b L D R hm _
...
theorem op_query (hm : m ∣ Fintype.card F) (b : Module.Basis (Fin t) (ZMod 2) F)
    ...
    (c : QLD.Content F m) (a : Answer F A m t d) :
    op L D R hm b hL (query hm b p c) a = sampleOp b L D R hm hL c p a := by
  cases p with
  | inl p =>
    change pauliOp b L D R hm (QLD.PauliCL.binaryQuestion b p
...
theorem check_query (hm : m ∣ Fintype.card F) (b : Module.Basis (Fin t) (ZMod 2) F)
    (p q : QuestionType QLD.Ty ℓ) (c : QLD.Content F m) (a a' : Answer F A m t d) :
    (game L D hm b).D (query hm b p c) (query hm b q c) a a' =
      sampleCheck b L D hm c p q a a' := by
  change TypedEstimates.questionCheck L (.pauli .X) (.pauli .Z) (project b) D (pauliCheck hm b)
...
theorem op_commute (hm : m ∣ Fintype.card F) (b : Module.Basis (Fin t) (ZMod 2) F) (hb : LowDegree.IsSelfDualBasis b)
    (hL : ∀ w, (L w).SupportedOn univ) (hR : R.IsPCC)
    (q r : Question m t ℓ) (h : 0 < (game (d := d) L D hm b).μ q r)
    (a a' : Answer F A m t d) :
  ...
  exact sampleOp_commute b L D R hb hm hL hR c p s he a a'

theorem op_reject (hm : m ∣ Fintype.card F) (b : Module.Basis (Fin t) (ZMod 2) F) (hb : LowDegree.IsSelfDualBasis b)
    (hd : 1 ≤ d) (hL : ∀ w, (L w).SupportedOn univ) (hR : R.IsPCC) (hv : R.value = 1)
    (q r : Question m t ℓ) (h : 0 < (game (d := d) L D hm b).μ q r)
    (a a' : Answer F A m t d) (hr : (game L D hm b).D q r a a' = false) :
    op L D R hm b hL q a * op L D R hm b hL r a' = 0 := by
  obtain ⟨c,p,s,he,rfl,rfl⟩ := game_positive_content L D hm b q r h
  rw [op_query, op_query]
  rw [check_query] at hr
  exact sampleOp_reject b L D R hb hm hd hL hR hv c p s he a a' hr
...
set_option backward.isDefEq.respectTransparency false in
theorem strategy_isPCC (hm : m ∣ Fintype.card F) (b : Module.Basis (Fin t) (ZMod 2) F) (hb : LowDegree.IsSelfDualBasis b)
    (hL : ∀ w, (L w).SupportedOn univ) (hR : R.IsPCC) :
    (strategy (d := d) L D R hm b hL).IsPCC := by
  intro p q h a a'
  have hh := congrArg (registerOp (Fintype.equivFin (Space L D R)).symm)
    (op_commute L D R hm b hb hL hR p.2 q.2 (doubled_positive L D hm b p q h).2 a a').eq
  simpa only [strategy, registerOp_mul] using hh

set_option backward.isDefEq.respectTransparency false in
theorem strategy_value (hm : m ∣ Fintype.card F) (b : Module.Basis (Fin t) (ZMod 2) F) (hb : LowDegree.IsSelfDualBasis b)
    (hd : 1 ≤ d) (hL : ∀ w, (L w).SupportedOn univ) (hR : R.IsPCC) (hv : R.value = 1) :
  ...
  have hh := congrArg (registerOp (Fintype.equivFin (Space L D R)).symm)
    (op_reject L D R hm b hb hd hL hR hv p.2 q.2 hμ a a' hd')
...
theorem exists_perfectPCC (hm : m ∣ Fintype.card F) (b : Module.Basis (Fin t) (ZMod 2) F) (hb : LowDegree.IsSelfDualBasis b)
    (hd : 1 ≤ d) (hL : ∀ w, (L w).SupportedOn univ) (hR : R.IsPCC) (hv : R.value = 1) :
    ∃ Q : SyncStrategy (game (d := d) L D hm b).doubled,
      Q.IsPCC ∧ Q.value = 1 ∧ Q.d = 2 * Fintype.card (Seed m t) * R.d := by
  refine ⟨strategy L D R hm b hL, strategy_isPCC L D R hm b hb hL hR,
    strategy_value L D R hm b hb hd hL hR hv, ?_⟩
