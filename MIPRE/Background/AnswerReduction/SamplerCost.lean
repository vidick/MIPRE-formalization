...
sampler, whose parameters the routine `parProg` computes (`MIPRE/Background/AnswerReduction/ArSampler`).
...
theorem progs_size_pdom (PD : PcpDecider) : ∃ c m e, ∀ {W X : ℕ} (sp : Prog) (lam mu sigma : ℕ),
  ...
  exact ⟨_, _, _, fun {W X} sp lam mu sigma hX hsp hl hs => by
    ...
    have h2 : esize (parProg PD lam mu sigma) =
      ...
      esize_hardcode _ _
    have h3 : (encode (lam, mu, sigma) : Data).size = esize lam + (esize mu + esize sigma + 1) + 1 :=
      by simp only [encode_prod, Data.size_cons]; rfl
    rw [h1, h2, h3]
...
theorem typedSampler_time (ℓ : ℕ) (PD : PcpDecider) (R : Polynomial ℕ) : ∃ C M E, ∀ {W : ℕ}
    (S : Sampler (ℓ + 1)) (lam mu sigma n : ℕ), ParamsBound PD R → 1 ≤ mu →
  ...
  obtain ⟨C, M, E, hP⟩ := ProductSampler.prog_time answerProg dimProg (ℓ + 1) cp mp ep cs ms es
    cO mO eO
  exact ⟨C, M, E, fun {W} S lam mu sigma n hR hmu hQ hl hs hn hSz hS d => by
    obtain ⟨r, t, hPD, hrun⟩ := hP (W := W) (K := mu) (OracleSampler.prog S.prog)
      ...
      (fun hX => hpp hR lam mu sigma n hX hmu hQ hl hs hn)
