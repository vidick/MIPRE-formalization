...
Upstream path: MIPStarRE/LDT/Commutativity/GCommStability/Scalar/Common.lean
-/
...
lemma scalar_pointwise_cauchy_schwarz_bound
    ...
    (x : Fq params)
    (hfirstR :
      ∑ g : Polynomial params, ev strategy.state (leftTensor (ι₂ := ι) (R.outcome g)) ≤ 1) :
    |∑ g : Polynomial params,
        ev strategy.state
  ...
  have hX_expand :
    ...
    calc
      ...
      _ = leftTensor (ι₂ := ι) (R.outcome g) := by
              rw [sqrt_subMeas_outcome_mul_self R g]
  have hY_expand :
      ∀ g : Polynomial params,
        (Y g)ᴴ * Y g =
