...
level, from a parameter `par`: the downsized PCP sampler of answer reduction is one. The product
...
one call, through the universal machine, to `A`'s:
...
The program is `hardcode core (encode (A.prog, ℓa, B.par))` for a core depending only on `B`'s
two functions: that is what makes a sampler built this way depend only on its input sampler's
...
/-- The input of a direct answer: `(par, n, type, kind, j, u, y)`, the query's kind, level and
vectors as in `Sampler.Query.toTuple`. -/
abbrev DirectInput := Data × ℕ × Data × ℕ × ℕ × BitStr × BitStr

/-- **A directly answered typed sampler**, the same for both players: a polynomial-time function
of a parameter `par` answers its queries, at every level. -/
structure DirectSampler (ℓ : ℕ) (T : Type*) [SizedEncoding T] where
  ...
  /-- The dimension, in unary, from the parameter and the index. -/
  dimProg : PolyTimeFun (Data × ℕ) Unary
  dimProg_eq : ∀ n, (dimProg (par, n)).length = dim n
  /-- The answers. -/
  answer : PolyTimeFun DirectInput BitStr
  answer_marginal : ∀ n t j z, 1 ≤ j → z.length = dim n →
    answer (par, n, encode t, 1, j, z, []) =
      toBits (((cl n t).truncate j).eval (ofBits (dim n) z))
  answer_linear : ∀ n t j u y, 1 ≤ j → (∃ x, u = toBits (((cl n t).truncate (j - 1)).eval x)) →
    y.length = dim n → answer (par, n, encode t, 2, j, u, y) =
  ...
  answer_factor : ∀ n t j u, 1 ≤ j → (∃ x, u = toBits (((cl n t).truncate (j - 1)).eval x)) →
    answer (par, n, encode t, 3, j, u, []) =
      indicatorBits ((cl n t).factorOfPrefix (j - 1) (ofBits (dim n) u))

namespace ProductSampler

variable (B : PolyTimeFun DirectInput BitStr) (D : PolyTimeFun (Data × ℕ) Unary)

/-! ## The core

Its input is `(H, n, q)` with `H = (A.prog, ℓa, par)` hardcoded. -/

/-- `A`'s program. -/
def progD : PolyTimeFun Data Data := treeHead.comp treeHead
/-- `A`'s level. -/
def levelA : PolyTimeFun Data ℕ := readNat.comp (treeHead.comp (treeTail.comp treeHead))
/-- `B`'s parameter. -/
def parD : PolyTimeFun Data Data := treeTail.comp (treeTail.comp treeHead)
/-- The index. -/
def nD : PolyTimeFun Data Data := treeHead.comp treeTail
/-- The typed query, `nil` for the dimension query. -/
def qD : PolyTimeFun Data Data := treeTail.comp treeTail
/-- The type pair. -/
...
def nN : PolyTimeFun Data ℕ := readNat.comp nD
/-- `B`'s dimension. -/
def dimB : PolyTimeFun Data Unary := D.comp (parD.pair nN)
...
def answerB : PolyTimeFun Data BitStr :=
  B.comp (parD.pair (nN.pair ((treeTail.comp tagD).pair (kind.pair (lev.pair
...
/-- The core of the product sampler. -/
def core : Prog := Prog.routeOneCall (route B D) selfUniversal.univ post

theorem core_closed : (core B D).WellScoped 1 :=
  Prog.routeOneCall_closed _ selfUniversal.closed post

/-- The product sampler's program: the core with `(A.prog, ℓa, par)` hardcoded. -/
def prog (ap : Prog) (ℓa : ℕ) (par : Data) : Prog := hardcode (core B D) (encode (ap, ℓa, par))

theorem prog_closed (ap : Prog) (ℓa : ℕ) (par : Data) : (prog B D ap ℓa par).WellScoped 1 :=
...
variable {Ta Tb : Type*} [SizedEncoding Ta] [SizedEncoding Tb] (ap : Prog) (ℓa : ℕ) (par : Data)
  (n : ℕ)

/-- The input of the core on a typed query. -/
abbrev input (q : TypedSampler.Query (Ta × Tb)) : Data :=
  .cons (encode (ap, ℓa, par)) (encode (n, q))

section Atype

variable (ta : Ta) (tb : Tb) (q : Sampler.Query)

local notation "X" => input ap ℓa par n (TypedSampler.Query.atType (ta, tb) q)
...
@[simp] theorem dimB_input : dimB D X = D (par, n) := by
  change D (par, readNat (encode n)) = _
  rw [readNat_encode]
@[simp] theorem qD_input : qD X = .cons (encode (ta, tb)) (encode q) := rfl
@[simp] theorem tagD_input : tagD X = .cons (encode ta) (encode tb) := rfl
@[simp] theorem parD_input : parD X = par := rfl
@[simp] theorem progD_input : progD X = encode ap := rfl
@[simp] theorem nD_input : nD X = encode n := rfl
@[simp] theorem nN_input : nN X = n := readNat_encode n

theorem answerB_input : answerB B D X =
    B (par, n, encode tb, q.toTuple.1, q.toTuple.2.2.1,
      ((q.toTuple.2.2.2.1).reverse.take (D (par, n)).length).reverse,
      ((q.toTuple.2.2.2.2).reverse.take (D (par, n)).length).reverse) := by
  simp only [answerB, rightBits, comp_apply, pair_apply, kind_input, lev_input, uD_input,
    yD_input, dimB_input, reverse_apply, take_apply, parD_input, nN_input, tagD_input,
    treeTail_cons]

theorem queryA_input (j : PolyTimeFun Data ℕ) : queryA D j X =
    .cons (encode n) (.cons (encode ta) (encode (q.toTuple.1, q.toTuple.2.1, j X,
      ((q.toTuple.2.2.2.1).reverse.drop (D (par, n)).length).reverse,
      ((q.toTuple.2.2.2.2).reverse.drop (D (par, n)).length).reverse))) := by
...
theorem route_dimension :
    route B D (input ap ℓa par n (TypedSampler.Query.dimension : TypedSampler.Query (Ta × Tb))) =
      (true, .cons (.cons (encode ap) (.cons (encode n) .nil))
        (.cons .nil (encode ((D (par, n)).map fun _ => true)))) := by
  have h : qD (input ap ℓa par n (TypedSampler.Query.dimension :
      TypedSampler.Query (Ta × Tb))) = Data.nil := rfl
  have hd : dimB D (input ap ℓa par n (TypedSampler.Query.dimension :
      TypedSampler.Query (Ta × Tb))) = D (par, n) := by
    change D (par, readNat (encode n)) = _
    rw [readNat_encode]
...
theorem route_atType (ta : Ta) (tb : Tb) (q : Sampler.Query) :
    route B D (input ap ℓa par n (TypedSampler.Query.atType (ta, tb) q)) =
      if q.toTuple.1 = 1 then
        request B D levA (input ap ℓa par n (TypedSampler.Query.atType (ta, tb) q))
      else if q.toTuple.2.2.1 ≤ ℓa then
        request B D lev (input ap ℓa par n (TypedSampler.Query.atType (ta, tb) q))
      else direct B D (input ap ℓa par n (TypedSampler.Query.atType (ta, tb) q)) := by
  have h : qD (input ap ℓa par n (TypedSampler.Query.atType (ta, tb) q)) ≠ Data.nil := by
    simp
  simp only [route, PolyTimeFun.ite_apply, ap₂_apply, treeEq_apply, const_apply, h,
...
theorem levA_input (ta : Ta) (tb : Tb) (q : Sampler.Query) :
    levA (input ap ℓa par n (TypedSampler.Query.atType (ta, tb) q)) =
...
theorem request_input (ta : Ta) (tb : Tb) (q : Sampler.Query) (j : PolyTimeFun Data ℕ) :
    request B D j (input ap ℓa par n (TypedSampler.Query.atType (ta, tb) q)) =
      (true, .cons (.cons (encode ap) (queryA D j
          (input ap ℓa par n (TypedSampler.Query.atType (ta, tb) q))))
        (.cons (.cons .nil .nil) (encode (answerB B D
          (input ap ℓa par n (TypedSampler.Query.atType (ta, tb) q)))))) := rfl

theorem direct_input (ta : Ta) (tb : Tb) (q : Sampler.Query) :
    direct B D (input ap ℓa par n (TypedSampler.Query.atType (ta, tb) q)) =
      (false, encode ((if q.toTuple.1 = 2 then
          ((q.toTuple.2.2.2.2).reverse.drop (D (par, n)).length).reverse
        else ((q.toTuple.2.2.2.1).reverse.drop (D (par, n)).length).reverse).map (fun _ => false)
        ++ answerB B D (input ap ℓa par n (TypedSampler.Query.atType (ta, tb) q)))) := by
...
theorem route_call_shape (H nD' q payload : Data) (h : route B D (.cons H (.cons nD' q)) =
    (true, payload)) : ∃ d ctx, payload = .cons (.cons (treeHead H) (.cons nD' d)) ctx := by
...
theorem prog_runs_call (x a ctx r : Data) (t : ℕ)
    (hroute : route B D (.cons (encode (ap, ℓa, par)) x) =
      (true, .cons (.cons (encode ap) a) ctx))
    (hr : ap.Runs a r t) : ∃ t', (prog B D ap ℓa par).Runs x (post (ctx, r)) t' := by
  ...
  obtain ⟨tc, hc⟩ := Prog.routeOneCall_indirect _ selfUniversal.closed post _ _ ctx r tu
    hroute hu
  exact ⟨_, hardcode_time (core_closed B D) hc⟩

theorem prog_runs_direct (x r : Data)
    (hroute : route B D (.cons (encode (ap, ℓa, par)) x) = (false, r)) :
    ∃ t, (prog B D ap ℓa par).Runs x r t := by
...
def prodDirect : TypedSampler L (Ta × Tb) where
  prog := ProductSampler.prog B.answer B.dimProg A.prog ℓa B.par
  ...
  runs_dimension n := by
    ...
    obtain ⟨t', h'⟩ := prog_runs_call B.answer B.dimProg A.prog ℓa B.par _ _ _ _ t
      (route_dimension (Ta := Ta) (Tb := Tb) B.answer B.dimProg A.prog ℓa B.par n) h
    rw [post_dim, List.length_map, B.dimProg_eq] at h'
    exact ⟨t', h'⟩
  runs_marginal n w t j z hj hL hz := by
    obtain ⟨ta, tb⟩ := t
    have hlen : z.length = A.dim n + (B.dimProg (B.par, n)).length := by
    ...
    obtain ⟨tA, hA'⟩ := A.runs_marginal n w ta j' (z.take (A.dim n)) hj1 hj2
      (by simp [hz])
    have hroute := route_atType B.answer B.dimProg A.prog ℓa B.par n ta tb (.marginal w j z)
    simp only [Sampler.Query.toTuple, if_true, request_input] at hroute
    have hq : queryA B.dimProg levA (input A.prog ℓa B.par n
      ...
      rfl
    rw [hq] at hroute
    obtain ⟨t', h'⟩ := prog_runs_call B.answer B.dimProg A.prog ℓa B.par _ _ _ _ tA hroute hA'
  ...
  runs_linear n w t j u y hj hL hu hy := by
    ...
    have hul : u.length = A.dim n + (B.dimProg (B.par, n)).length := by
      rw [B.dimProg_eq]; exact length_toBits _
    have hyl : y.length = A.dim n + (B.dimProg (B.par, n)).length := by
    ...
    have hB := B.answer_linear n tb j (u.drop (A.dim n)) (y.drop (A.dim n)) hj
      ⟨_, huB⟩ (by simp [hy])
    have hroute := route_atType B.answer B.dimProg A.prog ℓa B.par n ta tb (.linear w j u y)
    ...
    · simp only [Sampler.Query.toTuple, show (2 : ℕ) ≠ 1 by decide, if_false, hjl, if_true,
      ...
      have hq : queryA B.dimProg lev (input A.prog ℓa B.par n
    ...
    · simp only [Sampler.Query.toTuple, show (2 : ℕ) ≠ 1 by decide, if_false, hjl,
      ...
      obtain ⟨t', h'⟩ := prog_runs_direct B.answer B.dimProg A.prog ℓa B.par _ _ hroute
  ...
  runs_factor n w t j u hj hL hu := by
    ...
    have hroute := route_atType B.answer B.dimProg A.prog ℓa B.par n ta tb (.factor w j u)
    ...
    · simp only [Sampler.Query.toTuple, show (3 : ℕ) ≠ 1 by decide, if_false, hjl,
      ...
      exact h'
  halts n d := by
    have hcore : Halts (core B.answer B.dimProg)
        (.cons (encode (A.prog, ℓa, B.par)) (.cons (encode n) d)) := by
      cases h : route B.answer B.dimProg (.cons (encode (A.prog, ℓa, B.par)) (.cons (encode n) d))
        with
      | mk call payload =>
        cases call with
        | false =>
          ...
          exact ⟨payload, t, hr⟩
        | true =>
          obtain ⟨d', ctx, rfl⟩ := route_call_shape B.answer B.dimProg _ _ _ _ h
          ...
          exact ⟨_, tc, hc⟩
    obtain ⟨r, t, hr⟩ := hcore
    exact ⟨r, _, hardcode_time (core_closed _ _) hr⟩
