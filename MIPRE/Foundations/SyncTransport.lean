...
A game description denotes a *synchronous* game by construction, so it can match `V.game n T`
itself only where `𝒱_n` is synchronous (`MIPRE.Verifier.isSynchronousAt_of_game_matches`) —
and the class `B` of the halting reduction does not ask for synchronicity. So every bridge here
matches `g` against `V.doubledGame n T` (`Foundations/GapCompression.lean`, over
`Foundations/GameDouble.lean`), the game on `Bool × 𝒳` whose distribution avoids the diagonal
outright; `V.HasPerfectPCC n T` is a strategy of that game to begin with. Given a description
`g` whose distribution and decision predicate match those of the doubled game along
equivalences `eX`, `eA` of the alphabets:

* `Verifier.quantumValue_toGame_eq_valStar_doubled`: `quantumValue g.game = val*(𝒱_n)`, with no
  synchronicity hypothesis;
* `Verifier.exists_perfectPCC_syncGame_doubled`: `V.HasPerfectPCC n T` (blueprint `def:pcc`,
  the completeness hypothesis of `thm:compression`) gives a value-`1` PCC synchronous strategy
  for `g.syncGame`, by `MIPRE.SyncStrategy.relabel`;
* `Verifier.syncValue_syncGame_eq_one_doubled` and `Verifier.gameValue_toGame_eq_one_doubled`:
  hence `MIPRE.syncValue g.syncGame = 1`, hence `HaltingGameValue.gameValue g.toGame = 1`,
  through `HaltingGameValue.GameData.gameValue_eq_syncValue`;
* `Verifier.gameValue_toGame_le_of_valStar_le_doubled`: in the other direction `val*(𝒱_n) ≤ c`
  gives `gameValue g.toGame ≤ c`, since `synval ≤ val*` (blueprint `lem:sync-le-valstar`). This
  is the soundness half, and it needs no PCC strategy; `gameValue_toGame_le_of_valStar_le` is
  the same statement against `V.game n T` itself, kept because it holds and costs nothing.

These are the bridges that separate `MIPRE.Halting.halting_reduction`, which concludes in
`val*`, from `thm:halting` as the blueprint states it; `MIPRE.Halting.tab_value` and
`gameValue_tab_eq_one` are proved from them. Three un-doubled bridges that carried a PCC
strategy onto a description matching `𝒱_n` itself were removed when `HasPerfectPCC` moved to
the doubled game (issue #77): a strategy of the doubled game has no reason to agree with itself
across the two tags, so it does not restrict to a synchronous strategy of `𝒱_n`, and nothing
fed them.
-/
...
theorem exists_perfectPCC_syncGame_doubled
  ...
  rw [S.value_relabel g.syncGame eX eA hμ hD, hval]
